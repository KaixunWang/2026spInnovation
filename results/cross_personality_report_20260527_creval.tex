...
Rather than treating conflict as a homogeneous creativity elevator, we emphasize \textbf{dimensional splitting}: the same stressor can elevate novelty-like signals while eroding value and coherence support for $C_{\mathrm{auto}}{=}N_{\mathrm{auto}}{\cdot}V_{\mathrm{auto}}$, consistent with componential and divergent-thinking accounts of creativity~\cite{boden,amabile1983,guilford1967,runco} and with dual-process accounts that separate rapid associative production from slower monitoring of consistency and judgment~\cite{kahneman,mednick1962}.
...
The default pipeline uses \textbf{deterministic HuggingFace probes} (no LLM sampling): $S$ clips $(f{-}e)$ where $f$ is a RoBERTa formality probability~\cite{brooke2020} and $e$ is non-neutral emotion mass from a DistilRoBERTa emotion classifier~\cite{hartmann2020}; $R$ is the dot product of a StyleDistance sentence embedding~\cite{patel2024} with a fixed unit axis loaded from precomputed axis vectors (so register contrast is geometric rather than hand-labelled). An alternate configuration uses an LLM coordinate scorer that returns JSON $(S,R)$ on a 1--5 scale, averaged over repeated seeds with spread retained as a stability diagnostic---both backends normalize to $[-1,1]$ before conflict computation.
...
\textbf{Space-L} tests whether the hand-crafted Space-H corners are recoverable from text: per persona we generate $\geq 200$ reference passages (GPT-4o / GPT-4o mini~\cite{openai2024}), embed with SBERT~\cite{reimers2019,xiao2023}, PCA to $k{=}2$, and Procrustes-align~\cite{gower1975} to the four H persona vectors (\texttt{build\_space\_l}).
...
Implementation follows the public CrEval release (Qwen2.5-7B-Instruct~\cite{qwen2025} base with the CrEval-7b LoRA adapter served through a LLaMA-Factory OpenAI-compatible API~\cite{llamafactory2024,cao2025creval,creval_repo}). The model returns one of three outcomes for each pair: response 1 preferred, response 2 preferred, or tie. We convert these into a win-rate
\[
...
\bibitem{amabile1983}
T.~Amabile.
\newblock \textit{The Social Psychology of Creativity}.
\newblock Springer, 1983.

\bibitem{guilford1967}
J.~P.~Guilford.
\newblock \textit{The Nature of Human Intelligence}.
\newblock McGraw-Hill, 1967.

\bibitem{mednick1962}
S.~A.~Mednick.
\newblock The associative basis of the creative process.
\newblock \textit{Psychological Review} 69, 3 (1962), 220--227.
...
\bibitem{simonton1999}
D.~K.~Simonton.
\newblock \textit{Origins of Genius: Darwinian Perspectives on Creativity}.
\newblock Oxford University Press, 1999.

\bibitem{brooke2020}
J.~Brooke and G.~Hirst.
\newblock Native Language Identification and Formality.
\newblock In \textit{Proceedings of LREC 2020}, 6082--6092.

\bibitem{hartmann2020}
J.~Hartmann, S.~Sonnabend, and A.~Nevrkla.
\newblock Emotion English DistilRoBERTa-base.
\newblock Hugging Face model card, 2020.

\bibitem{patel2024}
A.~Patel, J.~Zhu, J.~Qiu, Z.~Horvitz, M.~Apidianaki, K.~McKeown, and C.~Callison-Burch.
\newblock StyleDistance: Stronger Content-Independent Style Embeddings with Synthetic Parallel Examples.
\newblock \textit{arXiv preprint arXiv:2410.12757}, 2024.

\bibitem{reimers2019}
N.~Reimers and I.~Gurevych.
\newblock Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks.
\newblock In \textit{Proceedings of EMNLP-IJCNLP 2019}, 3982--3992.

\bibitem{xiao2023}
S.~Xiao, Z.~Liu, P.~Zhang, and X.~Mu.
...
\newblock \textit{Psychometrika} 40, 1 (1975), 33--51.
...
\newblock \textit{arXiv preprint arXiv:2412.19437}, 2024.
...
\newblock In \textit{Proceedings of NAACL 2018}, 1112--1122.
...
\newblock In \textit{Advances in Neural Information Processing Systems 36 (NeurIPS 2023)}.

\bibitem{llamafactory2024}
Y.~Zheng, R.~Zhang, J.~Zhang, et al.
\newblock LLaMA-Factory: Unified Efficient Fine-Tuning of 100+ LLMs.
\newblock In \textit{Proceedings of ACL 2024 (System Demonstrations)}, 400--410.

\bibitem{bates2015}
D.~Bates, M.~M\"achler, B.~Bolker, and S.~Walker.
\newblock Fitting Linear Mixed-Effects Models Using \texttt{lme4}.
\newblock \textit{Journal of Statistical Software} 67, 1 (2015), 1--48.

\bibitem{lin1991}
J.~Lin.
\newblock Divergence measures based on the Shannon entropy.
\newblock \textit{IEEE Transactions on Information Theory} 37, 1 (1991), 145--151.
